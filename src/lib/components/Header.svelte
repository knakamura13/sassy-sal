...
<div class="header-container fixed top-0 left-0 right-0 z-50">
    <header
        class="flex items-center justify-between px-4 md:px-6 bg-gray-800 text-white transition-all duration-300 {compact
            ? 'h-12 bg-opacity-90 backdrop-blur-sm'
            : 'h-16'} w-full"
    >
        <div class="flex gap-4 items-center">
            <a href={$adminMode ? '/?admin=true' : '/'} class="flex gap-4 items-center cursor-pointer">
                ...
                <div
                    class="h-6 w-6 flex items-center justify-center transition-transform duration-300 {compact
                        ? 'scale-75'
                        : 'scale-100'}"
                >
                    {#if showLogo}
                        <img
                            alt="Site logo"
                            class="h-6 w-6 filter invert transition-opacity duration-100 {logoLoaded
                                ? 'opacity-100'
                                : 'opacity-0'}"
                            src={logoPath}
                            title="Site logo"
